    ...
    public partial class MainForm : RibbonForm
    {
        ...
        private void Initialize()
        {
            ...
            if (ParamManager.SystemParam.ProjectType == Convert.ToInt32(eProjectType.BLOWER))
            {
                ...
                ResultBaseWnd.ShowDummyWindowEvent += new MainResultBase.ShowDummyWindowHandler(ShowDummyWindow);
                ResultBaseWnd.EventInitialize();
                ResultBaseWnd.ClearResultData(true, "", ParamManager.SystemParam.InDataFolderPath, ParamManager.SystemParam.OutDataFolderPath);
            }
            else
        ...
        private bool LOTChange(string _LOTInfo)
        {
            ...
            try
            {
                if (_LOTName == "LotEnd")
                {
                    ResultBaseWnd.LOTEnd();
                    if(CheckDummyCount()) ResultBaseWnd.ClearResultData();
                    else                  ResultBaseWnd.ClearResultData(false);
                    _Result = true;
                }
                else
                {
                    ResultBaseWnd.ClearResultData(true, _LOTName, ParamManager.SystemParam.InDataFolderPath, ParamManager.SystemParam.OutDataFolderPath);
                    ...
                    //LDH, 2019.04.09, Vision 재로드시에는 Dummy창 open 안함
                    if (_LOTInfoTemp[0] != "@N") _Result = ShowDummyWindow();
                    else _Result = true;
                }
            }
            catch(Exception ex)
        ...
        //LDH, 2019.04.09, Dummy Window 열기
        private bool ShowDummyWindow()
        {
            bool _Result = false;

            try
            {
                DummyCountWindow _DummyCountWnd = new DummyCountWindow();
                _DummyCountWnd.SetDummyCountEvent += new DummyCountWindow.SetDummyCountHandler(ResultBaseWnd.SetOriginDummyCount);
                _DummyCountWnd.ShowWindow();
                _DummyCountWnd.SetDummyCountEvent += new DummyCountWindow.SetDummyCountHandler(ResultBaseWnd.SetOriginDummyCount);
            }
            catch
            {
                _Result = false;
                CLogManager.AddSystemLog(CLogManager.LOG_TYPE.INFO, String.Format("Main : ShowDummyWindow Exception!!!"), CLogManager.LOG_LEVEL.LOW);
        ...
        private bool CheckDummyCount()
        {
            ...
            if(!_DummyResult) CMsgBoxManager.Show("Dummy 수량을 확인하세요", "", true);
